...
export const productService = {
  /**
   * Delete a product with a reason attached (admin / WO use-case).
   * The reason is sent as a query parameter required by the backend for admin users.
   */
  async deleteProductWithReason(id: string, reason: string): Promise<void> {
  ...
  async uploadImage(productId: string, file: File): Promise<{ url: string }> {
    ...
    const res = await apiClient.post<{ url: string }>(
      `/v1/stock/products/${productId}/upload-image`,
      formData,
