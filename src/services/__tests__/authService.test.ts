...
describe('authService', () => {
  it('login posts credentials and returns token', async () => {
    mockPost.mockResolvedValue({ data: { accessToken: 't1', refreshToken: 'r1', accessTokenExpiresIn: '3600s', role: 'Customer', email: 'u@t.com' } });
  ...
  it('register returns accessToken', async () => {
    mockPost.mockResolvedValue({ data: { accessToken: 't1', refreshToken: 'r1', accessTokenExpiresIn: '3600s', role: 'Customer', email: 'n@t.com' } });
    const res = await authService.register({ email: 'n@t.com', password: 'p', username: 'New User' });
    expect(res.accessToken).toBe('t1');
  });

  it('externalLogin with Google', async () => {
    mockPost.mockResolvedValue({ data: { accessToken: 't1', refreshToken: 'r1', accessTokenExpiresIn: '3600s', role: 'Customer' } });
    await authService.externalLogin({ provider: 'Google', idToken: 'gt' });
    expect(mockPost).toHaveBeenCalledWith('/auth/external-login', { provider: 'Google', idToken: 'gt' });
  });
  ...
  it('getProfile returns User', async () => {
    mockGet.mockResolvedValue({ data: { id: 'u1', username: 'User', email: 'u@t.com', role: 'Customer', isActive: true, createdAt: '2024-01-01' } });
  ...
  it('resendVerification', async () => {
    ...
    const res = await authService.resendVerification({ email: 'u@t.com' });
    expect(res.message).toBe('Sent');
  });

  it('verifyEmail with token via query params', async () => {
