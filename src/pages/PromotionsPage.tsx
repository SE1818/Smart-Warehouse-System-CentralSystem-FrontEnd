...
function FlashSaleCard({
  promo, products, onDelete
}: { promo: PromotionDto; products: Product[]; onDelete: (id: string) => void }) {
    ...
    <div className={`bg-white rounded-2xl border shadow-sm hover:shadow-md transition-all overflow-hidden group ${isActive ? 'border-amber-200' : 'border-slate-200'}`}>
      ...
      <div className="px-5 py-3 border-t border-slate-100 flex justify-end">
        <button
          ...
          className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-bold rounded-lg border border-red-200/60 transition-all cursor-pointer"
        >
          Xóa
        </button>
      </div>
...
export function PromotionsPage() {
  ...
  // ── Flash Sale handlers ──
  const openCreateFlash = () => { setFlashForm(defaultFlashForm()); setSelProductId(''); setSelFlashPrice(0); setSelStockLimit(0); setShowFlashModal(true); };
  ...
  const handleSubmitFlash = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowFlashModal(false); setFlashForm(defaultFlashForm()); loadAll();
    } catch (err) { handleApiError(err, 'tạo Flash Sale'); }
  };
                ...
                {filteredFlash.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
                    {filteredFlash.map(fs => (
                      <FlashSaleCard key={fs.id} promo={fs} products={availableProducts} onDelete={setDeletingId} />
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full border border-slate-200/60 overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-amber-100 bg-gradient-to-r from-amber-50 to-orange-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-heading font-black text-slate-900">Tạo Flash Sale mới</h2>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Chương trình bán hàng với giá đặc biệt có giới hạn thời gian</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmitFlash} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input type="text" value={flashForm.code}
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 focus:bg-white transition-all text-sm font-bold" />
                </div>
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">Mô tả</label>
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
                <button type="button" onClick={() => setShowFlashModal(false)}
                  className="px-4 py-2.5 border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-xl text-xs font-bold transition-all cursor-pointer">Hủy</button>
                <button type="submit"
