...
const stockService = (await import('@/services/stock')).stockService as unknown as {
...
describe('WarehousesPage', () => {
  ...
  it('shows empty state when no warehouses', async () => {
    ...
    renderWarehousesPage();
    await waitFor(() => {
      expect(screen.getByText('Không tìm thấy kho hàng nào')).toBeDefined();
    });
  });
  ...
  it('shows error state when fetch fails', async () => {
    ...
    renderWarehousesPage();
    await waitFor(() => {
      expect(screen.getByText(/Không thể tải danh sách kho hàng/)).toBeDefined();
    });
  });
