...
export function StoreRegistrationsPage() {
  ...
  const fetchRegistrations = async () => {
    setLoading(true);
    try {
      const data = await storeService.listPendingRegistrations();
  ...
  const handleApprove = async (id: string, name: string) => {
    if (!window.confirm(`Bạn có chắc chắn muốn phê duyệt mở cửa hàng "${name}"?`)) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-sm text-slate-500 font-medium">Danh sách các cửa hàng đang chờ phê duyệt tài khoản và trạm làm việc</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-16 text-center border border-slate-100 shadow-sm flex flex-col items-center gap-4">
          ...
          <div className="space-y-1">
            <h3 className="text-base font-bold text-slate-800">Không có yêu cầu chờ duyệt</h3>
            <p className="text-slate-450 text-sm max-w-sm">Hiện tại không có bất kỳ đơn đăng ký mở cửa hàng mới nào đang chờ xử lý.</p>
          </div>
        </div>
      ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  ...
                  <th className="px-6 py-4 text-center">Thời gian đăng ký</th>
